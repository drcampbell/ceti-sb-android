...
public class UserSearchAndContactUser {

    @Rule
    public ActivityTestRule<LoginActivity> mActivityTestRule = new ActivityTestRule<>(LoginActivity.class);

    @Test
    public void userSearchAndContactUser() {
        ...
        ViewInteraction checkBox = onView(
                allOf(withId(R.id.user_checkBox), withText("Users"), isDisplayed()));
        checkBox.perform(click());

        ViewInteraction editText3 = onView(
                allOf(withId(R.id.searchText), isDisplayed()));
        editText3.perform(replaceText("babu"), closeSoftKeyboard());
        ...
        ViewInteraction button3 = onView(
                allOf(withId(R.id.contact_user_button), withText("Contact User"), isDisplayed()));
        button3.perform(click());

        ViewInteraction editText4 = onView(
                withId(R.id.message_content));
        editText4.perform(scrollTo(), replaceText("hi"), closeSoftKeyboard());

//        pressBack();

        ViewInteraction button4 = onView(
                allOf(withId(R.id.send_message_button), withText("Send Message"), isDisplayed()));
        button4.perform(click());

        ViewInteraction button5 = onView(
                allOf(withId(R.id.contact_user_button),
                        childAtPosition(
                                childAtPosition(
